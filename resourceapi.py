#
# Copyright 2020 Fraunhofer Institute for Software and Systems Engineerin
#
...
class ResourceApi:
    ...
    def put_data(self,artifact,data):
        return self.session.put(artifact + "/data",data)
    def get_artifacts_for_agreement(self, agreement):
        return json.loads(self.session.get(agreement + "/artifacts").text)

    def get_catalogs(self):
        return json.loads(self.session.get(self.recipient + "/api/catalogs").text)
    def get_catalog_resources(self, catalog):
        return json.loads(self.session.get(catalog + "/offers").text)

    def get_representations(self):
        return json.loads(self.session.get(self.recipient + "/api/representations").text)
